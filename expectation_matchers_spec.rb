describe 'Expectation Matchers' do
	...
	describe 'equivalence matchers' do
		...
		it 'will match loose equality with #eq' do
			...
			c = 17
			d = 17.0
			expect(c).to eq(d)				# different types, "close enough"
		end
	...
	describe 'other useful matchers' do
		...
		it 'will match object types' do
			...
			expect('test').to be_an_instance_of(String)	# alias ^

			expect('test').to be_kind_of(String)
			expect('test').to be_a_kind_of(String)			# alias ^
			expect('test').to be_a(String)							# alias ^
			expect([1,2,3]).to be_an(Array)							# alias ^
		end
	...
	describe 'predicate matchers' do
		...
		it 'will match be_* to custom methods ending in ?' do
			...
			# Can use these when methods end in "?", require no args,
			...
			expect(product.visible?).to be true 	# same thing
		end
	...
	describe 'observation matchers' do
		...
		# observes any errors raise by the block
